...
class CommandHandler(TextHandler):
    ...
    def on_data(self, session, byte_data):
        ...
        """
        try:
            print("message received: "+to_str(byte_data))
                ...
                try:
                    ...
                    for read_sock in readable:
                        try:
                            ...
                            #LOGGER.debug("DEBUG:creating session_thread %s", self._session_thread_class)
                            session_thread = self._session_thread_class(
                                self, self._data_handler, client_sock, client_address)
                            #LOGGER.debug("DEBUG:created session_thread %s", self._session_thread_class)
                            self._sessions.append(session_thread)
                            #LOGGER.debug("DEBUG:starting session_thread %s", self._session_thread_class)
                            session_thread.start()
                            #LOGGER.debug("DEBUG:started session_thread %s", self._session_thread_class)
                        except OSError as exc:
                    ...
                    for session_thread in self._sessions:
                        if session_thread.state == sitcpy.THREAD_STOPPED:
                            ...
                            #LOGGER.debug("closing session %s", str(session_thread.client_address))
                except OSError as exc:
